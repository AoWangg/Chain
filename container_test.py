...
def enterprise_report_goods_info():
    ...
    for j in range(51):
        ...
        if (j + 1) % 10 == 0:
            end_time = time.time()
            time_list.append(round((end_time - start_time) * 1000, 2))

            x_axis.append(j)
            y_axis.append((end_time - start_time) * 1000 / j)

    # 绘图
    plt.hist(y_axis, bins=10)
    plt.xlabel('Average Delay (in ms)')
    plt.ylabel('Frequency')
    plt.title('CallContract Delay Distribution')

    # 将图表保存到当前目录下
    filename = "call_contract_delay.png"
    plt.savefig(os.path.join(os.getcwd(), filename))
        # out = call_contract(goods_Addr, "Im_goods", "getGoodsReports", args=[str(container.account_addr)],
        #                     signer=signer)
